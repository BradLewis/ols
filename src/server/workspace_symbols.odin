...
get_workspace_packages :: proc(
	config: ^common.Config,
	pkgs: ^[dynamic]string,
	allocator := context.temp_allocator,
) -> bool {
	for workspace in common.config.workspace_folders {
		uri := common.parse_uri(workspace.uri, allocator) or_return
		dirs := make([dynamic]string, 0, allocator)

		w := os.walker_create(uri.path)
		defer os.walker_destroy(&w)
		for info in os.walker_walk(&w) {
			if info.type == .Directory {
				dir, _ := filepath.replace_separators(info.fullpath, '/', allocator)
		...
		_pkg: for pkg in dirs {
			matches, err := filepath.glob(fmt.tprintf("%v/*.odin", pkg), allocator)
			...
			for exclude_path in common.config.profile.exclude_path {
				exclude_forward, _ := filepath.replace_separators(exclude_path, '/', allocator)
