...
lookup_builtin_symbol :: proc(index: ^Indexer, name: string, current_file_uri: string) -> (Symbol, bool) {
	if symbol, ok := lookup_symbol(index, name, "$builtin", current_file_uri); ok {
		return symbol, true
	}
...
@(private = "file")
lookup_symbol :: proc(index: ^Indexer, name: string, pkg: string, current_file_uri: string) -> (Symbol, bool) {
	if symbol, ok := memory_index_lookup(&index.index, name, pkg); ok {
		current_pkg := get_package_from_filepath(current_file_uri)
